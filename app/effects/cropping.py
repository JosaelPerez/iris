...
class CroppingConfiguration(BaseModel):
    ...
    def check_origin(cls, origin: Point) -> 'Point':
        if origin.x < 0 or origin.y < 0:
            raise ValueError(f"Origin point ({origin.x},{
                             origin.y}) must be greater or equal to 0.")
        return origin
    ...
    def check_configuration(self) -> 'CroppingConfiguration':
        if self.aspect_ratio == AspectRatio.FREE:
            ...
            if self.dimensions.width < minimun_cropping_size or self.dimensions.height < minimun_cropping_size:
                raise ValueError(f"Dimensions = {self.dimensions.width}x{self.dimensions.height} must be greater or equal to {
                                 minimun_cropping_size}x{minimun_cropping_size} pixels.")

        else:
            if self.dimensions.height:
                raise ValueError(f"Default aspect ratio '{
                                 self.aspect_ratio.value}' cannot be used with a custom height.")

            if self.dimensions.width < minimun_cropping_size:
                raise ValueError(f"Width dimension = '{
                                 self.dimensions.width}' must be greater or equal to {minimun_cropping_size} pixels.")

            if self.aspect_ratio == AspectRatio.SQUARE:
                if self.orientation is not None:
                    raise ValueError(f"Default aspect ratio '{
                                     self.aspect_ratio.value}' cannot be used with an orientation.")
                ...
                if self.orientation is None:
                    raise ValueError(f"Default aspect ratio '{
                                     self.aspect_ratio.value}' requires an orientation.")
                ...
                match (self.aspect_ratio, self.orientation):
                    ...
                    case _:
                        raise ValueError(f"Aspect ratio '{self.aspect_ratio.value}' with orientation '{
                                         self.orientation}' is not supported.")

                if self.dimensions.height < minimun_cropping_size:
                    raise ValueError(f"Height dimension = '{
                                     self.dimensions.height}' must be greater or equal to {minimun_cropping_size} pixels.")
