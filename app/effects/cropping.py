...
from app.effects.utils import Point, Dimension, Orientation, AspectRatio
...
class CroppingConfiguration(BaseModel):
    ...
    def check_origin(cls, origin: Point) -> "Point":
        if origin.x < 0 or origin.y < 0:
            raise ValueError(
                f"Origin point ({origin.x},{origin.y}) must be greater or equal to 0."
            )
        return origin
    ...
    def check_configuration(self) -> "CroppingConfiguration":
        if self.aspect_ratio == AspectRatio.FREE:
            if not self.dimensions.width or not self.dimensions.height:
                raise ValueError(
                    f"Free cropping requires both width and height dimensions."
                )

            if (
                self.dimensions.width < minimun_cropping_size
                or self.dimensions.height < minimun_cropping_size
            ):
                raise ValueError(
            ...
            if self.dimensions.height:
                raise ValueError(
                    f"Default aspect ratio '{self.aspect_ratio.value}' cannot be used with a custom height."
                )

            if self.dimensions.width < minimun_cropping_size:
                raise ValueError(
                    f"Width dimension = '{self.dimensions.width}' must be greater or equal to {minimun_cropping_size} pixels."
                )

            if self.aspect_ratio == AspectRatio.SQUARE:
                if self.orientation is not None:
                    raise ValueError(
                        f"Default aspect ratio '{self.aspect_ratio.value}' cannot be used with an orientation."
                    )
                ...
                if self.orientation is None:
                    raise ValueError(
                        f"Default aspect ratio '{self.aspect_ratio.value}' requires an orientation."
                    )
                ...
                match (self.aspect_ratio, self.orientation):
                    case (AspectRatio.THREE_TWO, self.orientation.LANDSCAPE):
                        self.dimensions.height = int(self.dimensions.width * 2 / 3)

                    case (AspectRatio.THREE_TWO, self.orientation.PORTRAIT):
                        self.dimensions.height = int(self.dimensions.width * 3 / 2)

                    case (AspectRatio.FOUR_THREE, self.orientation.LANDSCAPE):
                        self.dimensions.height = int(self.dimensions.width * 3 / 4)

                    case (AspectRatio.FOUR_THREE, self.orientation.PORTRAIT):
                        self.dimensions.height = int(self.dimensions.width * 4 / 3)

                    case (AspectRatio.FIVE_FOUR, self.orientation.LANDSCAPE):
                        self.dimensions.height = int(self.dimensions.width * 4 / 5)

                    case (AspectRatio.FIVE_FOUR, self.orientation.PORTRAIT):
                        self.dimensions.height = int(self.dimensions.width * 5 / 4)

                    case (AspectRatio.SIXTEEN_NINE, self.orientation.LANDSCAPE):
                        self.dimensions.height = int(self.dimensions.width * 9 / 16)

                    case (AspectRatio.SIXTEEN_NINE, self.orientation.PORTRAIT):
                        self.dimensions.height = int(self.dimensions.width * 16 / 9)

                    case (AspectRatio.SIXTEEN_TEN, self.orientation.LANDSCAPE):
                        self.dimensions.height = int(self.dimensions.width * 10 / 16)

                    case (AspectRatio.SIXTEEN_TEN, self.orientation.PORTRAIT):
                        self.dimensions.height = int(self.dimensions.width * 16 / 10)

                    case _:
                        raise ValueError(
                            f"Aspect ratio '{self.aspect_ratio.value}' with orientation '{self.orientation}' is not supported."
                        )

                if self.dimensions.height < minimun_cropping_size:
                    raise ValueError(
                        f"Height dimension = '{self.dimensions.height}' must be greater or equal to {minimun_cropping_size} pixels."
                    )
    ...
    try:
        if height:
            return CroppingConfiguration(
                ...
                dimensions=Dimension(width=width, height=height),
